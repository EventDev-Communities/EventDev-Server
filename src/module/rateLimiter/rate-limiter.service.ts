...
export class RateLimiterService {
  ...
  getAuthPolicy() {
    return 'Possíveis configurações/abordagens de controle para o domínio de autenticação'
  }
  ...
  async checkSlidingWindow(policy: IRateLimitPolicyData, banKey: string, zsetKey: string) {
    ...
    const now = Date.now()
    const pathAbsolute = path.join(__dirname, '../', '../../../.scripts/lua/rate-limiting.lua')
  ...
  async checkRestrictions(identity: string, domain: AvailableDomainsType, pourpose: string) {
    ...
    if (banTtl > 0) throw new ForbiddenException(`Sujeito banido temporariamente, tente novamente em: ${banTtl}s`)
