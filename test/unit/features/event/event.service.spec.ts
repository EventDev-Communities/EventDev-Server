...
describe('EventService', () => {
  ...
  describe('create', () => {
    ...
    it('should rethrow BadRequestException', async () => {
      ...
      mockCommunityService.isExistCommunity.mockRejectedValue(new BadRequestException('Bad Request'))
    ...
    it('should wrap unknown errors in BadRequestException', async () => {
      ...
      mockCommunityService.isExistCommunity.mockRejectedValue(new Error('Unknown error'))

      await expect(service.create(1, createEventDto)).rejects.toThrow(BadRequestException)
      await expect(service.create(1, createEventDto)).rejects.toThrow('Erro interno ao criar evento')
    })
  })

  describe('getById', () => {
    it('should return event if exists', async () => {
      mockEventRepository.getByID.mockResolvedValue({ id: 1 })
  ...
  describe('getAll', () => {
    it('should return paginated response', async () => {
      mockEventRepository.getAll.mockResolvedValue({ data: [], total: 0 })
    ...
    it('should pass filters to repository', async () => {
      mockEventRepository.getAll.mockResolvedValue({ data: [], total: 0 })
      const filters = { communityId: 1, modality: 'ONLINE', isActive: true }
      await service.getAll(10, 0, filters)
      expect(mockEventRepository.getAll).toHaveBeenCalledWith(10, 0, filters)
    })
  })
  ...
  describe('update', () => {
    it('should update event successfully', async () => {
      ...
      mockEventRepository.getByID.mockResolvedValue({ id: 1 })
      mockEventRepository.update.mockResolvedValue({ count: 1 })
    ...
    it('should update address if provided', async () => {
      ...
      mockEventRepository.getByID.mockResolvedValue({ id: 1 })
      mockEventRepository.update.mockResolvedValue({ id: 1 })

      await service.update(1, updateDto as any, 10)
      expect(mockAddressService.update).toHaveBeenCalledWith(updateDto.address, 10)
    })

    it('should throw BadRequestException if event data is missing', async () => {
      mockEventRepository.getByID.mockResolvedValue({ id: 1 })
      await expect(service.update(1, {} as any, 1)).rejects.toThrow(BadRequestException)
    })

    it('should throw NotFoundException if event does not exist', async () => {
      mockEventRepository.getByID.mockResolvedValue(null)
      await expect(service.update(1, { event: {} }, 1)).rejects.toThrow(NotFoundException)
    })
  })
  ...
  describe('delete', () => {
    it('should delete event successfully', async () => {
      mockEventRepository.getByID.mockResolvedValue({ id: 1 })
      mockEventRepository.delete.mockResolvedValue({ id: 1 })
