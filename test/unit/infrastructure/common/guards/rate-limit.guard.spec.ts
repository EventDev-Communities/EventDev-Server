...
describe('RateLimitGuard', () => {
  ...
  describe('canActivate', () => {
    ...
    it('should use user id for identity when user is authenticated', async () => {
      const customPolicy = {
        domain: 'auth' as const,
        windowMs: 60000,
        limit: 5,
      ...
      expect(result).toBe(true)
      expect(rateLimiterService.resolveIdentity as unknown as jest.Mock).toHaveBeenCalled()
      // Since we mocked implementation to return extractor(req), and extractor returns user:id
      expect(rateLimiterService.resolveIdentity as unknown as jest.Mock).toHaveReturnedWith('user:user-123')
    })

    it('should use default ban duration if not provided', async () => {
      const config = { domain: 'test', limit: 5, windowMs: 60000 }
      jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(config)

      await guard.canActivate(mockContext)

      expect(rateLimiterService.enforcePolicyCheckSlidingWindow as unknown as jest.Mock).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ banDurationMs: 300000 }),
        'test'
      )
    })

    it('should use IP for identity if not authenticated', async () => {
      const config = { domain: 'test', limit: 5, windowMs: 60000 }
      jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(config)
    ...
    it('should use "unknown" for identity if no IP and not authenticated', async () => {
      const config = { domain: 'test', limit: 5, windowMs: 60000 }
      jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(config)
      mockRequest.user = undefined
