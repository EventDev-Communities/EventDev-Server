...
describe('RateLimiterService', () => {
  ...
  describe('enforcePolicyCheckSlidingWindow', () => {
    ...
    it('should throw ForbiddenException if user is in deny list', async () => {
      const identity = 'test-user'
      const policy = {
        domain: 'global' as const,
        windowMs: 60000,
      ...
      redisMock.exists.mockResolvedValue(1) // Denied

      await expect(service.enforcePolicyCheckSlidingWindow(identity, policy, 'global'))
        .rejects
        .toThrow('Acesso bloqueado permanentemente')
    })

    it('should throw ForbiddenException if user is temporarily banned', async () => {
      const identity = 'test-user'
      const policy = {
        domain: 'global' as const,
        windowMs: 60000,
      ...
      redisMock.exists.mockResolvedValue(0)
      redisMock.ttl.mockResolvedValue(100) // Banned for 100s

      await expect(service.enforcePolicyCheckSlidingWindow(identity, policy, 'global'))
        .rejects
        .toThrow('Bloqueio temporário ativo')
    })
  })
  ...
  describe('onApplicationShutdown', () => {
    it('should do nothing if redis status is end', async () => {
      redisMock.status = 'end'
      await service.onApplicationShutdown()
      expect(redisMock.quit as unknown as jest.Mock).not.toHaveBeenCalled()
    })

    it('should quit redis gracefully', async () => {
      redisMock.status = 'ready'
      await service.onApplicationShutdown()
      expect(redisMock.quit as unknown as jest.Mock).toHaveBeenCalled()
    })

    it('should disconnect if quit fails', async () => {
      redisMock.status = 'ready'
      ;(redisMock.quit as unknown as jest.Mock).mockRejectedValue(new Error('Quit failed'))

      await service.onApplicationShutdown()

      expect(redisMock.quit as unknown as jest.Mock).toHaveBeenCalled()
      expect(redisMock.disconnect as unknown as jest.Mock).toHaveBeenCalled()
    })
  })
})
